    ...
    message = [
        {
            ...
            "content": """You are an assistant for an IoT system that
                ...
                You must only respond with a single word (roomON, roomOFF, kitchenON, kitchenOFF)
...
def send_command_to_esp(command):
    try:
        # Add a newline character to ensure ESP can detect the end of the command
        command_with_terminator = command + "\n"
        ser.write(command_with_terminator.encode())
        # print(f"Sent command to ESP: {command_with_terminator}")
    except Exception as e:
